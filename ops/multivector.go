...
// Wire codecs and handlers for the late-interaction (multi-vector / MaxSim)
// op family: vector_mv_create_collection, vector_mv_add, vector_mv_search,
// vector_mv_delete, vector_mv_drop_collection.
...
func handleMVCreate(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, cfg, err := DecodeMVCreateArgs(args)
	if err != nil {
		return nil, err
	}
...
func handleMVDrop(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, _, err := DecodeMVDeleteArgs(args)
	if err != nil {
		return nil, err
	}
...
func handleMVAdd(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, docID, tokens, meta, expected, hasExpected, keyTTLMs, sparse, err := DecodeMVAddArgsCASKeyTTLSparse(args)
	if err != nil {
		return nil, err
	}
...
func handleMVAddIfAbsent(tx *TxContext, args []byte) ([]byte, error) {
	...
	// Version-aware decode (old EncodeMVAddArgs wire ⇒ version 0). version!=0 →
	...
	name, docID, tokens, meta, version, keyExpires, sparse, err := DecodeMVAddArgsVersionedKeyExpiresSparse(args)
	if err != nil {
		return nil, err
	}
...
// version-preserving reinsert (EncodeVectorInsertArgsVersioned). Result: nil.
...
func handleMVExists(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, docID, err := DecodeMVExistsArgs(args)
	if err != nil {
		return nil, err
	}
	...
	return EncodeExistsResult(exists), nil
}
...
func handleMVSearch(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, query, k, candPerToken, _, _, filter, _, err := DecodeMVSearchArgsOptsFilter(args)
	if err != nil {
		return nil, err
	}
	...
	return EncodeMVResults(res), nil
}
...
// EncodeHybridResults (id + distance + SCORE — the fused score must survive the
...
func handleMVHybridSearch(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, query, sparseQ, k, opts, _, _, _, err := DecodeMVHybridArgs(args)
	if err != nil {
		return nil, err
	}
...
// fan-out). It shares the EncodeMVHybridArgs wire with vector_mv_hybrid_search; the
// lanes op returns the two pre-fusion lanes via EncodeHybridLanesResult instead of
...
func handleMVGetConfig(tx *TxContext, args []byte) ([]byte, error) {
	...
	return EncodeMVCreateArgs(name, idx.Config()), nil
}
...
func handleMVScanVectors(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, err := DecodeMVScanArgs(args)
	if err != nil {
		return nil, err
	}
	...
	return EncodeMVScanResult(recs), nil
}
...
// (EncodeVectorDocs — id + payload, no score, no token vectors; vector_mv_get is
...
func handleMVScroll(tx *TxContext, args []byte) ([]byte, error) {
	...
	col, filter, limit, _, _, afterID, hasAfter, order, err := DecodeMVScrollArgsOrder(args)
	if err != nil {
		return nil, err
	}
...
func handleMVDelete(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, docID, expected, hasExpected, err := DecodeMVDeleteArgsCAS(args)
	if err != nil {
		return nil, err
	}
...
func handleMVGet(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, docID, flags, err := DecodeVectorGetArgs(args)
	if err != nil {
		return nil, err
	}
	...
	return EncodeMVGetResultV(ok, tokens, payload, withVec, withPayload, version), nil
}
...
func handleMVGetBatch(tx *TxContext, args []byte) ([]byte, error) {
	...
	withVec := flags&getFlagWithVector != 0
	withPayload := flags&getFlagWithPayload != 0
	rows := make([]MVGetBatchRow, 0, len(ids))
	for _, id := range ids {
		tokens, payload, version, ok, err := tx.vectors.MVGetVersion(col, id)
		if err != nil {
			return nil, err
		}
		row := MVGetBatchRow{ID: id, Found: ok}
		if ok {
			if withVec {
				row.Tokens = tokens
			}
	...
	return EncodeMVGetBatchResult(rows), nil
}
...
func handleMVSetPayload(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, docID, meta, keyTTLMs, expected, hasExpected, err := DecodeSetPayloadArgsCAS(args)
	if err != nil {
		return nil, err
	}
	cas := vector.CASCond{Expected: expected, Has: hasExpected}
...
func handleMVDeletePayloadKeys(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, docID, keys, expected, hasExpected, err := DecodeDeletePayloadKeysArgsCAS(args)
	if err != nil {
		return nil, err
	}
...
func handleMVClearPayload(tx *TxContext, args []byte) ([]byte, error) {
	...
	name, docID, expected, hasExpected, err := DecodeClearPayloadArgsCAS(args)
	if err != nil {
		return nil, err
	}
