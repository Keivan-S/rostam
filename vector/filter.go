...
// Compile walks the filter once and returns a Predicate closure. A zero/empty
// filter compiles to a nil Predicate — callers treat nil as "match all" and
// skip evaluation entirely (the search hot path relies on this).
func (f Filter) Compile() (Predicate, error) {
	if f.IsZero() {
		return nil, nil
	}
	return f.compile()
...
func (f Filter) compileLeaf() (Predicate, error) {
	if f.Field == "" {
