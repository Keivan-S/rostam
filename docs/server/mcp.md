...
| `recall` | `query` (required), `namespace` (default `"default"`), `k` (default 5), `filter` (optional, ANDed with the namespace) | BM25-only or hybrid dense+BM25, per the embedder mode. Returns `{hits: [{id, content, score, key, created, updated, metadata}]}` — `key`, `created`, and `updated` are omitted on a hit with no key (a plain, unkeyed `remember`). |
